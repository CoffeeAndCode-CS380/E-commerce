...
import static com.hirbod.ecommerceproject.UserUtilityFile.isUserNameValid;


public class LoginController implements Initializable{

    @FXML
    private Label loginMessageLabel; //label to show error message
    @FXML
    ...
    public void loginButtonOnAction (ActionEvent event){
        // check if user nae is valid
        if(!isUserNameValid(enterUsernameTextField.getText())){
            loginMessageLabel.setText("Username must have at leasr one letter");
            return;
        }
        ...
        if (enterUsernameTextField.getText().isBlank() == false && enterPasswordField.getText().isBlank() == false){
            ...
            loginMessageLabel.setText("Please fill in all the fields");
            return;//to keep the error message and label's text dynamic
        }
    }
}